...
class mydata(Dataset):
    def __init__(self, LR_path, HR_path, in_memory = True, transform = None):
        ...
        if in_memory:
            ...
            self.HR_img = [np.array(Image.open(os.path.join(self.HR_path, hr)).convert("RGB")).astype(np.uint8) for hr in self.HR_img]
    ...
    def __getitem__(self, i):
        ...
        if self.in_memory:
            HR = self.HR_img[i].astype(np.float32)
            LR = self.LR_img[i].astype(np.float32)
            
        else:
            HR = np.array(Image.open(os.path.join(self.HR_path, self.HR_img[i])).convert("RGB"))
            LR = np.array(Image.open(os.path.join(self.LR_path, self.LR_img[i])).convert("RGB"))

        img_item['HR'] = (HR / 127.5) - 1.0
        img_item['LR'] = (LR / 127.5) - 1.0
                
        if self.transform is not None:
            img_item = self.transform(img_item)
            
        img_item['HR'] = img_item['HR'].transpose(2, 0, 1).astype(np.float32)
