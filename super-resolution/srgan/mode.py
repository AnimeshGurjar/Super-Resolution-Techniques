...
def train(args):
    ...
    dataset = mydata(HR_path = args.HR_path, LR_path = args.LR_path, in_memory = args.in_memory, transform = transform)
    print(dataset)
    loader = DataLoader(dataset, batch_size = args.batch_size, shuffle = True, num_workers = args.num_workers)
    
    generator = Generator(img_feat = 3, n_feats = 64, kernel_size = 2, num_block = args.res_num, scale=args.scale)
...
def test(args):
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    dataset = mydata(HR_path = args.HR_path, LR_path = args.LR_path, in_memory = False, transform = None)
